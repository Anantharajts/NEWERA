...
if (isset($_POST["edit"]) && $_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $delete_id = $_POST['rowid'];

    $delete = "UPDATE `payment_method_add` SET `IsDeleted`=1 WHERE `Id`=$delete_id";
    if (mysqli_query($con, $delete)) {
        // echo "deleted";
        echo "<script>window.location.href='payment_method_add.php'</script>";
    } else {
        echo "delete function error";
    }
}
            ...
            <form action="pay_method_action.php" method="post" onsubmit="return valid()">
                ...
                <div class="col row" style="gap:20px;flex-direction: column;text-align: center;">
                    ...
                    <div class="col">
                        ...
                        <input type="text" name="payment" id="pay_methodid" placeholder="Add Payment Method" value="<?php echo $p_name;?>" oninput="removevalidatiopn('pay_methodid')" style="padding:10px 5px;border-radius:3px;border:1px solid black;width:100%;">
                    </div>
                    <div class="col">
                        <button type="submit" name="submit" style="padding:10px 5px;border-radius:3px;background-color:black;color:white;width:100%;"><?php echo $p_id == 0? "ADD":"UPDATE"?></button>
                    </div>
            ...
            <div class="col" style="background-color:white;padding:15px;border-radius:10px;border-left:1px solid black;margin-top: 30px;">
                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                        ...
                        <?php
                        $stment = "SELECT `Id`, `Paymenrt_Method` FROM `payment_method_add` WHERE `IsDeleted`=0";
                        ...
                        if (mysqli_num_rows($data) > 0) {
                            ...
                            while ($_result = mysqli_fetch_assoc($data)) {
                                ...
                                $payid = $_result['Id'];
                                $paymethod_na = $_result['Paymenrt_Method'];
                        ?>

                                <tr>

                                    <td><?php echo $sl; ?></td>
                                    <td><?php echo $paymethod_na; ?></td>
                                    <td>
                                        <form action="payment_method_add.php" method="post">
                                            <div class="col row">
                                                <input type="hidden" name="rowid" id="row_id" value="<?php echo $payid; ?>">
                                                <input type="hidden" name="row_na" id="row_na" value="<?php echo $paymethod_na; ?>">
                                                <div class="col">
                                                ...
                                                <div class="col">
                                                    <button type="submit" name="delete" onclick="return confrime('Are you sure you want to delete....?')" style="background-color:white;border:none;"><svg
